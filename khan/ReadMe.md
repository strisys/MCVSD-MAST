...
- Again, recall that when dealing with x- and y-intercepts, the y- and x-coordinates are 0, respectively, in the ordered pair.  As such, to determine the answers to the two-variable questions given, we know that either x or y is 0, depending on the intercept in question.
  $$
  \begin{aligned}
  -5x - 4y = 10\\
  \text{y-intercept} (?,?)\\
...
- Remember, slope is rise ($$\Delta{y}$$) / run ($$\Delta{x}$$).  Since they are showing a graph, create a triangle at a point on the graph where both x and y are whole numbers, if possible, and then take the ratio of y given x.  Its essential to note you can test the answer by multiplying the slope by x to get y and looking at the graph to see if it makes sense.
